...
\begin{description}
   ...
   \item[Definizione \refstepcounter{definition}\thedefinition\label{def:categoria-notevole} – Categoria:] \label{sec:cat}
  ...
  Le principali categorie comprendono: Urbanistica, Ambiente, Sicurezza, Cultura e Istruzione, Innovazione e Digitale, Sociale e Inclusione, Mobilità Sostenibile.
...
È responsabile della creazione degli account speciali (moderatori, associazioni/comitati), della supervisione dei processi di verifica e della configurazione dei parametri operativi del sistema.
  ...
  Espressione anonima di consenso o dissenso da parte di un cittadino autenticato tramite SPID o CIE.
...
\begin{itemize}
  \item \textbf{Prioritizzazione data-driven}: La piattaforma aiuta a individuare rapidamente le iniziative più rilevanti per la comunità, riducendo il divario tra bisogni territoriali e agenda amministrativa.

  \item \textbf{Flusso operativo tracciabile}: Flusso end-to-end con stati, cronologia e registri pubblici di azioni e decisioni.

  \item \textbf{Sportello unico digitale}: Riduzione di canali frammentati e carico sugli uffici grazie a una piattaforma web unica.

  \item \textbf{Analisi tecnica}: Simulazioni e indicatori per valutare impatti economici, sociali e ambientali.

  \item \textbf{Open Data}: Dashboard, report esportabili e API per la pubblicazione di dati aggregati.
\end{itemize}

\subsection{Per i cittadini}

\begin{itemize}
  \item \textbf{Partecipazione effettiva}: Pubblicare, votare e co-progettare le proposte rende i cittadini protagonisti, orientando in modo concreto le priorità pubbliche.

  \item \textbf{Accesso semplice}: La piattaforma è accessibile via web senza installazioni.

  \item \textbf{Accessibilità}: Interfaccia conforme a WCAG 2.1 AA e multilingua.

  \item \textbf{Chiarezza sul percorso}: Ogni proposta riporta in modo trasparente il proprio stato (\textit{bozza}, \textit{in valutazione}, \textit{accettata}, \textit{rifiutata}),  garantendo tracciabilità e riducendo l’asimmetria informativa.

  \item \textbf{Tracciabilità personale}: Ogni cittadino può seguire le proprie proposte, modifiche e voti espressi anche con notifiche su aggiornamenti.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Definizione dei criteri di priorità}: Stabilire parametri oggettivi per la valutazione delle proposte è complesso. La formula di ranking richiede un monitoraggio continuo e revisioni periodiche, come previsto dal Requisito Non Funzionale sulla neutralità algoritmica (\ref{ref:neutralita}).

  \item \textbf{Neutralità e bias cognitivi}: Le decisioni automatizzate e i comportamenti collettivi possono introdurre distorsioni involontarie. Si prevede la revisione annuale degli algoritmi e l’audit di neutralità per garantire imparzialità e trasparenza dei processi.

  \item \textbf{Digital divide}: Una parte della popolazione può trovarsi in difficoltà nell’uso di strumenti digitali. Sono previsti punti di facilitazione presso biblioteche e sportelli comunali, supporto telefonico e assistenza tramite delega per l’inserimento delle proposte.

  \item \textbf{Competenza multi-ente}: Alcune proposte ricadono su enti diversi dal Comune di Trento (Provincia, Stato, gestori di servizi pubblici). Il sistema deve prevedere un flusso chiaro e tracciabile di reindirizzamento e presa verso l’ente competente.
  ...
  \item \textbf{Sostenibilità operativa ed economica}:
  Il mantenimento della piattaforma richiede risorse continuative per moderazione, assistenza, comunicazione, sicurezza e aggiornamenti software. È raccomandata la previsione di un piano di sostenibilità tecnica ed economica pluriennale.
