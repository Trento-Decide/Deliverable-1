...
L'utente che intende registrarsi come cittadino deve accedere inizialmente con SPID o CIE, il sistema richiede l'inserimento di email, password e nome utente (credenziali locali); inoltre l’utente può opzionalmente acconsentire a ricevere notifiche via email (abilita/disabilita).  Al termine, il sistema invia all'utente una email di conferma.
...
La pubblicazione è possibile solo se tutti i campi obbligatori previsti risultano compilati.  
...
\begin{itemize}
  	\item visualizzare un elenco personale contenente le proprie proposte, sia pubblicate che in stato di bozza che eliminate;
...
\subsection{Segnalazione dei contenuti da parte dei cittadini}
...
\subsection{Revisione dei contenuti segnalati} \label{sec:validazione-contenuti}
Il sistema deve consentire ai moderatori di visualizzare la coda delle segnalazioni, accedere al dettaglio del contenuto e della motivazione e decidere se confermarne la pubblicazione o procedere alla rimozione.

\subsection{Moderazione dei contenuti pubblicati}
Il sistema deve consentire ai moderatori di visualizzare l’elenco dei contenuti pubblicati e di rimuovere quelli che non risultano conformi ai criteri di utilizzo configurati dall’amministrazione.

\subsection{Rimozione dei contenuti}
...
\subsubsection{Limitazioni sui cittadini}
...
\subsubsection{Limitazioni sulle associazioni}
...
In caso di rilevamento, il sistema deve inserire l’elemento nella coda di revisione o sospenderne temporaneamente la pubblicazione.
...
Il sistema deve fornire all’amministrazione una dashboard contenente indicatori aggregati: numero utenti attivi, proposte per categoria, voti per zona.
...
\subsection{Modifica dello stato di una proposta}
