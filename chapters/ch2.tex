...
\subsection{Registrazione dei cittadini} \label{rf:registrazionecittadini}
Il sistema deve consentire la registrazione dei cittadini tramite SPID o CIE. Al termine dell’autenticazione, il sistema richiede all’utente di impostare indirizzo email, password e nome utente.

Gli utenti possono registrarsi come:
\begin{itemize}
\item residenti nel Comune, previa verifica automatica della residenza tramite ANPR;
\item domiciliati nel territorio comunale per studio o lavoro, indicando il motivo del domicilio (studio o lavoro) e un indirizzo nel Comune.
\end{itemize}

Per gli utenti domiciliati, il sistema deve verificare che l’indirizzo appartenga al territorio comunale.

Il sistema deve inoltre consentire all’utente di esprimere il consenso alla ricezione di notifiche email. Al termine della procedura viene inviata un’email di conferma, l'utente ha 10 minuti dalla conferma dei dati per abilitare il proprio account.
...
Se l'utente modifica email o password il sistema invalida il cookie di autenticazione (se esistente).
...
\subsubsection{Scartare proposta} \lavel{rf:scartarebozza}
...
Il sistema deve consentire all'utente di salvare una proposta nello stato di \textit{bozza}. La bozza verrà contrassegnata con tale stato e sarà visibile (assieme alle altre bozze) solo all’autore all’interno della lista personale delle proposte. Inoltre, selezionando una bozza, il sistema riaprirà il modulo di compilazione per consentirne la modifica e/o la pubblicazione.

\subsection{Pubblicazione proposta} \label{rf:pubblicazioneproposta}
Il sistema deve consentire all'utente di pubblicare una proposta precedentemente creata o salvata in stato di \textit{bozza}.  
...
Il sistema deve consentire ad associazioni e comitati di presentare \textit{proposte collettive} \defref{def:propostacollettiva}, etichettate come tali.
...
\begin{itemize}
  ...
  \item di visualizzare i sondaggi pubblicati dall’amministrazione (\ref{rf:sondaggi}).
\end{itemize}
...
Il sistema deve consentire agli utenti autenticati di esprimere un voto positivo o negativo su:

\begin{itemize}
    \item \textbf{Proposte} — il voto può essere modificato fino alla definizione dell’esito (\textit{accettata} o \textit{rifiutata});
...
\begin{itemize}
	\item aggiungere o rimuovere proposte e sondaggi dai \textit{preferiti};
...
Il sistema deve analizzare automaticamente i contenuti generati dagli utenti per individuare linguaggio inappropriato, spam o duplicati.

In caso di rilevamento, il sistema deve:
\begin{itemize}
    \item impedire temporaneamente la pubblicazione del contenuto oppure nasconderlo dalla visualizzazione pubblica;
    \item inserire il contenuto nella coda di revisione dei moderatori (\ref{rf:validazionecontenuti});
\end{itemize}

La moderazione automatica non deve effettuare rimozioni definitive né applicare limitazioni agli account. Le decisioni con effetto permanente devono essere convalidate da un moderatore o dall’amministrazione secondo le procedure previste.
...
\section{Policy Simulator e modelli statistici} \label{rf:polsimemodstat}

\subsection{Simulazione di scenari di policy} \label{rf:policysimulator}
...
\subsection{Analisi predittiva e statistica} \label{rf:analpredestat}
Il sistema deve consentire all’amministrazione di generare stime sugli effetti futuri delle proposte sulla base di dataset caricati nel sistema.
...
\subsection{Visualizzare policy simulation} \label{rf:visualizzazionesimulazione}
...
Ogni sondaggio deve includere almeno:
(a) \textit{Titolo},
(b) \textit{Descrizione},
(c) \textit{Categoria},
...
Il sistema deve consentire all’amministratore di redigere, aggiornare e pubblicare il report di valutazione associato a una proposta, includendo testo descrittivo, allegati e indicatori quantitativi/qualitativi. Al momento della pubblicazione, il sistema deve permettere opzionalmente la modifica dello stato della proposta.