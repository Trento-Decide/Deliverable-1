...
{
  \item Interfaccia: sviluppo del modulo per l'inserimento di email, password, username e preferenza notifiche.

  \item Database: Implementazione dei controlli di unicità per email e username nel database.

  \item Logica: integrazione dei servizi di autenticazione SPID e CIE.

  \item Logica: collegamento con ANPR per la verifica della residenza.

  \item Logica: configurazione del servizio di SMTP per l'invio delle email.

  \item Logica: far scadere il token di verifica dopo 10 minuti.
}
...
}
{
  ...
  \item Database: che associa il voto dell'utente all'elemento, garantendo l'unicità e anonimità del voto.
}

\section{Proporre modifiche a una proposta~\ref{rf:modificacollaborativa}} \label{us:modificacollaborativa}
\userstory
{
    Come cittadino, voglio poter proporre modifiche alle proposte di terzi per favorirne una co-creazione trasparente.
}
{
    \item Un utente deve poter inviare una proposta di modifica per una proposta di un terzo.

    \item Le modifiche devono entrare in uno stato attesa di approvazione dell'autore.

    \item L'autore della proposta deve poter accettare o rifiutare le modifiche.

    \item Ogni modifica approvata deve generare una nuova versione consultabile con le precedenti.

    \item La pubblicazione di una nuova versione deve causare l'azzeramento dei voti.

    \item L'autore deve poter ripristinare una versione precedente.
}
{
    \item Interfaccia: differenze tra la versione corrente e la modifica proposta.

    \item Database: relazione 1-a-n tra Proposta e Proposte di modifica/Versioni.

    \item API: invio di proposte di modifica e approvazione da parte dell'autore.

    \item Logica: azzeramento dei voti e cambio di versione corrente.

    \item Logica: funzionalità di ripristino.
}
...
{
  \item Interfaccia: componente UI sulle card e nelle pagine di dettaglio.

  \item Interfaccia: pagina della lista dei preferiti.

  \item Logica: pulizia dei preferiti in caso di cancellazione dell'elemento.
}
...
{
  \item Interfaccia: bottone "Segnala" nell'interfaccia delle proposte.

  \item Interfaccia: modulo con la lista predefinita di motivazioni e campo di testo.

  \item API: creazione dell'oggetto Segnalazione collegato a Utente e Contenuto.

  \item Logica: inserimento nella coda di revisione.
}
